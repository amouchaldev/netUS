<div class="px-3 py-4 bg-white rounded mb-3 d-none post" data-aos="fade-down">
        ...
        <div class="d-flex flex-column">
            <a href="{{ route('users.show', $post->user->username) }}"
                ...
                <div class="card-avatar overflow-hidden">
                    <img src="{{ asset('storage/' . $post->user->avatar) }}" alt="" class="w-100 h-100">
                </div>
                <h6 class="mb-0 ms-2">
                    {{ $post->user->name }}
                </h6>
            ...
            <div class="d-flex flex-wrap align-items-center">
                <a class="text-reset text-decoration-none me-2"
                    href="{{ route('communities.index', $post->community->slug) }}"><i
                        class="fa-solid fa-bars fa-xs"></i> <small>{{ $post->community->name }}</small></a>
                <small class="me-2"><i class="fa-regular fa-clock fa-sm"></i>
                    {{ $post->created_at->diffForHumans() }}</small>
                <small class="me-2"><i class="fa-regular fa-eye fa-sm"></i> {{ $post->number_visites }}</small>
                <small class="me-2"><i class="fa-regular fa-comment fa-sm"></i> {{ $post->comments_count }}
                    comments</small>
                {{-- more --}}
                {{-- @auth --}}
                @if (Route::currentRouteName() == 'posts.show' && (Gate::check('update', $post) || Gate::check('delete', $post)))
                    <div class="dropdown d-inline">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="moreDropDown">
                            @can('update', $post)
                                <li><a class="dropdown-item" href="{{ route('posts.edit', $post->slug) }}">edit</a>
                                </li>
                            @endcan
                            <li><span id="delete-post" class="dropdown-item"
            ...
            @if (Route::currentRouteName() != 'posts.show')
                <h5 class="mt-3"><a href="{{ route('posts.show', $post->slug ?? null) }}"
    ...
    {{-- <div class="truncate-last-comments">
        {!! $post->body !!}
    </div> --}}
