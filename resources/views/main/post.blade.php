...
@section('content')
    <div class="row">
        ...
        <div class="col-12">
            ...
            <h1 class="col-lg-8 h3 my-4"><a href="{{ route('posts.show', $post->slug) }}"
                    class="text-decoration-none">{{ $post->title }}</a></h1>
            <div class="row">
                ...
                <div class="col-12 col-lg-8 mb-5 mb-lg-0">
                    <div class="row">
                        <div class="col-12" id="posts">
                            ...
                            @auth
                                {{-- comment form --}}
                                <x-create-form route='comments.store' :post="$post"></x-create-form>
                            @endauth
                            ...
                            <ul class="list-group">
                                @foreach ($alsoLearn as $post)
                                    <li class="list-group-item list-group-item-action"><a
                                            ...
                                            class="text-dark text-decoration-none">{{ $post->title }}</a></li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
    ...
    @if(Gate::check('update', $post))
        ...
        editComment.forEach(el => {
            ...
            el.addEventListener('click', e => {
                console.log('yow')
                let commentBody = e.target.parentElement.parentElement.parentElement.parentElement
                    .parentElement.parentElement.nextElementSibling

                commentBody.innerHTML =
                    `<textarea class="tinymce-editor" id="comment-form" name="body">${commentBody.innerHTML}</textarea>`
                initCommentForm()
                ...
                setTimeout(() => {
                    document.addEventListener('click', function(event) {
                        ...
                        if (outsideClick) {
                            if (commentBody.contains(commentBody.querySelector(
                                    'textarea'))) {
                                console.log(tinymce.get("comment-form").getContent())

                                commentBody.innerHTML = tinymce.get("comment-form")
                                    .getContent()
                                //   console.log('===>', commentBody.querySelector('textarea').innerHTML)
                                _updateComment(e.target.getAttribute('data-id'), tinymce
                                    .get("comment-form").getContent())
